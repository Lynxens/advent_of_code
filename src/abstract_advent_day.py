...
def expected_answers(example_answer: Answer | tuple[Answer, ...] | bool = False, answer: Answer | tuple[Answer, ...] | bool = False):
...
class AbstractAdventDay(unittest.TestCase):
    ...
    def read_example(self, example_index: int|None = None) -> object:
        if example_index is None:
            return self.read(f'input_example.txt')
        else:
            return self.read(f'input_example_{example_index}.txt')

    def read_main(self) -> object:
        return self.read(f'input.txt')

    def test_puzzle_1_example(self):
        if self.puzzle_1.example_answer == False:
            return
